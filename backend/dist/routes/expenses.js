...
router.get('/', [
    ...
    (0, express_validator_1.query)('category').optional().isIn(['FOOD', 'TRAVEL', 'OFFICE', 'COMMUNICATIONS', 'OTHER']),
...
router.post('/', [
    ...
    (0, express_validator_1.body)('originalCurrency').isIn(['INR', 'USD', 'EUR', 'GBP']),
    (0, express_validator_1.body)('category').isIn(['FOOD', 'TRAVEL', 'OFFICE', 'COMMUNICATIONS', 'OTHER']),
    (0, express_validator_1.body)('subcategory').trim().isLength({ min: 1 }),
    (0, express_validator_1.body)('description').trim().isLength({ min: 1 }),
    (0, express_validator_1.body)('expenseDate').isISO8601(),
    (0, express_validator_1.body)('tags').optional().isArray(),
    (0, express_validator_1.body)('location').optional().isObject(),
    (0, express_validator_1.body)('receiptData').optional().isObject()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const { amount, originalCurrency, category, subcategory, description, expenseDate, tags = [], location, receiptData } = req.body;
        // Convert currency using external API service
        let convertedAmount = amount;
        let exchangeRate = 1.0;
        if (originalCurrency !== 'INR') {
            try {
                const conversionResult = await externalAPI_1.externalAPIService.convertCurrency(amount, originalCurrency, 'INR');
...
router.put('/:id', [
    ...
    (0, express_validator_1.body)('category').optional().isIn(['FOOD', 'TRAVEL', 'OFFICE', 'COMMUNICATIONS', 'OTHER']),
    (0, express_validator_1.body)('subcategory').optional().trim().isLength({ min: 1 }),
