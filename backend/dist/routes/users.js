...
router.get('/', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'), async (req, res) => {
    try {
...
router.put('/:id', [
    ...
    (0, express_validator_1.body)('role').optional().isIn(['ADMIN', 'MANAGER', 'EMPLOYEE']),
], async (req, res) => {
    try {
...
router.post('/invite', [
    auth_1.authenticateToken,
    (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'),
    ...
    (0, express_validator_1.body)('role').isIn(['MANAGER', 'EMPLOYEE']),
    (0, express_validator_1.body)('tempPassword').isLength({ min: 8 })
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
